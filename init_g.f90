...
!    if (exist) read (unit=input_unit("init_g_knobs"), nml=init_g_knobs)
  ...
  subroutine ginit_default
    ...
    complex, dimension (:,:,:,:,:), allocatable :: f0, gyro
    complex, dimension (:,:,:,:), allocatable :: field_f0
    ...
    allocate(g_0(naky,nakx,-nzgrid:nzgrid,ntubes,vmu_lo%llim_proc:vmu_lo%ulim_alloc))
    g_0 = 0.
    allocate(f0(naky,nakx,-nzgrid:nzgrid,ntubes,vmu_lo%llim_proc:vmu_lo%ulim_alloc))
    f0 = 0.
    allocate(field_f0(naky,nakx,-nzgrid:nzgrid,ntubes))
    field_f0 = 0.
    ...
    !! initial condition in vmu- This is used to compare the g_0 part of g = g_bar + g_0e^(gamma*t), but is not used to calculate frequency
    !! f0 is the initial consition on g
    !! g_0 is the eigenmode
    do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_alloc
       iv = iv_idx(vmu_lo,ivmu)
       imu = imu_idx(vmu_lo,ivmu)
       is = is_idx(vmu_lo,ivmu)
       do iz = -nzgrid, nzgrid
          f0 (:,:,iz,ia,ivmu) = exp(-(zed(iz)/width0)**2) &
    ...
    call advance_fields (f0, field_f0, apar, dist = 'gbar')

    do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_alloc
       iv = iv_idx(vmu_lo,ivmu)
       imu = imu_idx(vmu_lo,ivmu)
       is = is_idx(vmu_lo,ivmu)
       call gyro_average (field_f0, ivmu, gyro(:,:,:,:,ivmu))
       g_0 (:,:,:,ia,ivmu) = spec(is)%zt*gyro(:,:,:,ia,ivmu) &
            * maxwell_vpa(iv,is)*spread(spread(maxwell_mu(ia,:,imu,is),1,naky),2,nakx)&
            * maxwell_fac(is)
    end do

    !! Set initial condition in kxkyz (as is done in the original default
    gvmu = 0.    
    do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
       iz = iz_idx(kxkyz_lo,ikxkyz)
       ikx = ikx_idx(kxkyz_lo,ikxkyz)
       iky = iky_idx(kxkyz_lo,ikxkyz)
       is = is_idx(kxkyz_lo,ikxkyz)
       gvmu(:,:,ikxkyz) = spread(maxwell_mu(ia,iz,:,is),1,nvpa)*spread(maxwell_vpa(:,is),2,nmu)*maxwell_fac(is)&
            * exp(-(zed(iz)/width0)**2)
    end do
    
    deallocate(f0, field_f0)
    deallocate(gyro)
