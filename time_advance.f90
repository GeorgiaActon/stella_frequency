  ...
  subroutine advance_stella (istep, adjoint)
    ...
    if (present(adjoint)) then
       if (mod(istep,2)==1) then
          ...
          call advance_implicit (istep, phi, apar, gnew, adjoint)
       else
          call advance_implicit (istep, phi, apar, gnew, adjoint)
          call advance_explicit (gnew, adjoint)
       end if
...
!    complex, dimension (:,:,-nzgrid:), intent (in out) :: phi, apar
  ...
  subroutine solve_gke (gin, rhs_ky, restart_time_step)
    ...
    integer :: ivmu,imu,iv,is, ia, iky,ikx
    ...
    do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
       ...
       rhs_ky(:,:,:,ia,ivmu) = rhs_ky(:,:,:,ia,ivmu) + spec(is)%zt*0.01*code_dt &
            *maxwell_vpa(iv,is)*spread(spread(maxwell_mu(ia,:,imu,is),1,naky),2,nakx)&
            *maxwell_fac(is)*gyro_phi(:,:,:,ia,ivmu)
    end do
    deallocate(gyro_phi)
...
!     end if
  ...
  subroutine solve_adjoint (gin, rhs, restart_time_step)
    ...
    complex, dimension (:,:,-nzgrid:,:,vmu_lo%llim_proc:), intent (in) :: gin
    complex, dimension (:,:,-nzgrid:,:,vmu_lo%llim_proc:), intent (out), target :: rhs
    logical, intent (out) :: restart_time_step
    ...
    restart_time_step = .false.
    if (.not.restart_time_step) then
       if(include_mirror) call advance_mirror_explicit (gin, rhs)
  ...
  subroutine add_omega_term (gin, src)
    ...
    do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
       src(:,:,:,:,ivmu) = src(:,:,:,:,ivmu) &
            - spread(spread(omega_g, 3, nztot),4, ntubes)*gin(:,:,:,:,ivmu)*code_dt
    end do
  ...
  subroutine add_adjoint_field (gin, src)
    ...
    fields_updated = .false.
    allocate(gyro_phi(naky,nakx,-nzgrid:nzgrid,ntubes,vmu_lo%llim_proc:vmu_lo%ulim_proc))
    gyro_phi = 0.

    phi = 0.
    apar = 0.
    call advance_fields (gin, phi, apar, dist='gbar', adjoint=adjoint)
    apar = 0.
