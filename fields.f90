...
!    real, dimension (:,:,:), allocatable :: numerator
    ...
    if (fphi > epsilon(0.0)) then
       ...
       do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
          ...
          !! new definition of gamtot
          g0 = spread(aj0v(:,ikxkyz)**2,1,nvpa) &
               * spread(maxwell_vpa(:,is),2,nmu)*spread(maxwell_mu(ia,iz,:,is),1,nvpa)*maxwell_fac(is)
          !! old definition of gamtot
          !spread((1.0 - aj0v(:,ikxkyz)**2),1,nvpa) &
               !* spread(maxwell_vpa(:,is),2,nmu)*spread(maxwell_mu(ia,iz,:,is),1,nvpa)*maxwell_fac(is)
  ...
  subroutine advance_fields (g, phi, apar, dist, adjoint)
    ...
    if (fields_kxkyz) then
       ...
       if (present(adjoint)) then
          write(*,*) 'ABORT!!!!'
!          call get_adjoint_fields (gvmu, phi, apar, dist)
       else
          call get_fields (gvmu, phi, apar, dist)
       end if
    else
       if (present(adjoint)) then
          call get_adjoint_fields_vmulo (g, phi, apar, dist)
       else
...
!        deallocate (g0)
  ...
  subroutine get_adjoint_fields_vmulo (g, phi, apar, dist)

    use stella_layouts, only: vmu_lo
    use stella_layouts, only: imu_idx, is_idx, iv_idx
    use gyro_averages, only: gyro_average
    use kt_grids, only: nakx, naky, aky
    ...
    real, dimension (:,:,:), allocatable :: eta2
    real, dimension (:,:), allocatable :: g0
    real :: wgt, tmp
    ...
    allocate (eta2(naky,nakx,-nzgrid:nzgrid))
    eta2 = 0.
    allocate (g0(nvpa,nmu))
    do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
       it = it_idx(kxkyz_lo,ikxkyz)
       ...
       wgt = spec(is)%z*spec(is)%z*spec(is)%dens_psi0/spec(is)%temp
       call integrate_vmu (g0, iz, tmp)
       eta2(iky,ikx,iz) = eta2(iky,ikx,iz) + tmp*wgt
    end do
    call sum_allreduce (eta2)
    ...
    do it = 1, ntubes
       do iz = -nzgrid, nzgrid
          do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
             ...
             !star(it,iz,ivmu) = -zi*0.01* maxwell_vpa(iv,is)*maxwell_mu(it,iz,imu,is)*maxwell_fac(is)
             multg = spread(aky,2,nakx)*star(it,iz,ivmu)/spec(is)%zt - omega_g(:,:) &
                  *spread(spread(maxwell_vpa(iv,is)*maxwell_mu(ia,iz,imu,is)&
                  *maxwell_fac(is),1,naky),2,nakx)
             call gyro_average (g(:,:,iz,it,ivmu), iz, ivmu, gyro_g(:,:,ivmu))
             gyro_g(:,:,ivmu) = gyro_g(:,:,ivmu)*multg(:,:)
          end do
          call integrate_species (gyro_g, iz, spec%zt, phi(:,:,iz,it),reduce_in=.false.)
       end do
    end do
    deallocate(gyro_g)
    
    call sum_allreduce(phi)
    phi = phi/spread(eta2,4,ntubes)
!    phi = phi/eta
    apar = 0.
...
!     end if
  ...
  subroutine get_dchidy_4d (phi, apar, dchidy)
    ...
    do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
       is = is_idx(vmu_lo,ivmu)
       iv = iv_idx(vmu_lo,ivmu)
       field = spread(spread(spread(aky,2,nakx),3,2*nzgrid+1),4,ntubes) &
